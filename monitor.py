...
class Monitor:
    ...
    def run_remote_job(
        ...
        ci_link = "/".join([ci_link, next_build_number])
        try:
        ...
        if build_info["result"] in ["SUCCESS", "UNSTABLE"]:
            ...
            try:
                ...
                patch_exec(
                    eid=eid, c3_link=report, ci_link=ci_link, status="PASSED"
                )
            except (re.error, TypeError):
                print(f"Test job {job} was success, but report not found")
                patch_exec(
                    eid=eid, c3_link="", ci_link=ci_link, status="FAILED"
                )

        elif not build_info["result"]:
            print(f"Test job {job} was Timeout")
            patch_exec(eid=eid, c3_link="", ci_link=ci_link, status="FAILED")
    ...
    def start(self):
        ...
        for data in self._data:
            ...
            if rev == -1:
                ...
                continue
            print(
                f'snap: {snap_item["name"]} channel: {snap_item["channel"]} revision: {rev} '
            )
